...
class AssemblyInput(object):
    ...
    def __setup(self):
        ...
        self.__formDefList = [('assem_id', 'a_id_', self.__placeHolderValue),
    ...
    def __getPdbxStructAssemblyGenDepositorInfo(self, modelFile, container):
        ...
        #
        for myD in assemGen:
            if (not 'chain_id_list' in myD) or (not myD['chain_id_list']):
                continue
            #
            newAsymList = []
            newList = []
    ...
    def makeSymopInfoTable(self, entryId="1ABC", entryFileName=None):
        ...
        if (not 'space_group_name_H-M' in myD) or (not myD['space_group_name_H-M']):
            return not_found_msg
        #
        myLabel = { 'space_group_name_H-M' : 'Space Group', 'length_a' : 'a', 'length_b' : 'b', 'length_c' : 'c', \
    ...
    def makeDepositorAssemblyDetailsTable(self, entryId="1ABC", entryFileName=None):
        ...
        """
        oL = []
        sbAttList = [('id', 'ID'), ('details', 'Details'), 
        ...
        attList = [('id', 'ID'), ('assembly_id', 'Assembly'), 
                   ('experimental_support', 'Exp. Support'), 
                   ('details', 'Details')]
        self.__displayTable(oL, assemEvidence, 
                            'pdbx_struct_assembly_auth_evidence', entryId, 
                            '', '', attList)

        # pdbx_struct_assembly_auth_classification
        attList = [('assembly_id', 'Assembly ID'), ('reason_for_interest', 'Reason of Interest')]
        self.__displayTable(oL, assemClassification, 
                            'pdbx_struct_assembly_auth_classification', entryId, 
                            '', '', attList)

        # pdbx_struct_assembly_depositor_info
        attList = [('id', 'ID'), ('details', 'Details'), 
                   ('method_details', 'Method details'), 
                   ('oligomeric_count', 'Olig. Count'),
                   ('oligomeric_details', 'Olig. Details'), 
                   ('matrix_flag', 'Matrices provided'), 
                   ('upload_file_name', 'Upload file name')]
        self.__displayTable(oL, assemL, 'pdbx_struct_assembly_depositor_info', 
                            entryId, iC, iS, attList)
        ...
        self.__displayTable(oL, assemGen, 
                            'pdbx_struct_assembly_gen_depositor_info', 
                            entryId, '', '', attList)
        # pdbx_struct_oper_list_depositor_info
        attList = [('id', 'ID'), ('name', 'Name'), 
                   ('symmetry_operation', 'Op'), ('type', 'Type'), 
                   ('matrix[1][1]', '[1][1]'), ('matrix[1][2]', '[1][2]'), 
                   ('matrix[1][3]', '[1][3]'),
                   ('matrix[2][1]', '[2][1]'), ('matrix[2][2]', '[2][2]'), 
                   ('matrix[2][3]', '[2][3]'),
                   ('matrix[3][1]', '[3][1]'), ('matrix[3][2]', '[3][2]'), 
                   ('matrix[3][3]', '[3][3]'),
                   ('vector[1]', 'T1'), ('vector[2]', 'T2'), ('vector[3]', 'T3')]
