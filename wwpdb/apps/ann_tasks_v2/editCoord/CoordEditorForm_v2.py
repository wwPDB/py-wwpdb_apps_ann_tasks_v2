...
class CoordEditorForm(object):
    ...
    def __setup(self):
        ...
        #
        self.__listItemTemplate = '<li><a id="%s" class="discontrol ui-corner-all" href="#">' \
    ...
    def __generateEntryJson(self):
        ...
        #
        self.__runScript()
        if os.access(os.path.join(self.__jsonPath, self.__entryId + ".json"), os.F_OK) and \
           os.access(os.path.join(self.__sessionPath, self.__entryId + "_chainids.txt"), os.F_OK) and \
           os.access(os.path.join(self.__sessionPath, self.__entryId + "_index.cif"), os.F_OK):
            return True
        #
        return False
    ...
    def __topChainHtml(self, mol_idx, chainID):
        ...
        for Type in ( "Polymer", "Nonpolymer", "Water" ):
            if not Type in pickleObj:
                continue
            #
            if int(pickleObj[Type]["Label"][3]) > 1:
                label = labelTemplates % ( Type, pickleObj[Type]["Label"][0], pickleObj[Type]["Label"][1], \
                                                 pickleObj[Type]["Label"][2], pickleObj[Type]["Label"][3] )
            else:
                label = labelTemplate % ( Type, pickleObj[Type]["Label"][0], pickleObj[Type]["Label"][1], \
                                                pickleObj[Type]["Label"][2], pickleObj[Type]["Label"][3] )
            #
    ...
    def __typeChainHtml(self, Type, mol_idx, chainID, Identifier):
        ...
        """
        pickleObj,ok = self.__readChainPickleFile(mol_idx, chainID)
        if (not ok) or (not Type in pickleObj):
            myD = {}
            myD["statuscode"] = "failed"
        ...
        if Type == "Polymer":
            labelPolymerTemplate = "<br/>Renumbering Polymer using Range: For the residue renumbering please make sure the range covers the full length of " \
    ...
    def __residueHtml(self, mol_idx, chainID, residueID, Identifier):
        ...
        """
        pickleObj,ok = self.__readChainPickleFile(mol_idx, chainID)
        if (not ok) or (not Identifier in pickleObj) or (not "Atoms" in pickleObj[Identifier]):
            myD = {}
            myD["statuscode"] = "failed"
