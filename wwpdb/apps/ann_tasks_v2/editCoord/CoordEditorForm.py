...
class CoordEditorForm(object):
    ...
    def __runScript(self):
        ...
        f.write('${BINPATH}/DepictMolecule -input ' + self.__entryFile + \
                ' -output ' + self.__entryId + '_html.txt ' + \
                ' -output_2 ' + self.__entryId + '_chainids.txt ' + \
                ' -output_3 ' + self.__entryId + '_index.cif ' + \
                ' -log ' + self.__entryId + '_summary.log\n')
        f.write('#\n')
        f.close()
        cmd = 'cd ' + self.__sessionPath + '; chmod 755 ' + self.__entryId + '_script.csh; ' \
            + ' ./' + self.__entryId + '_script.csh >& summary_log'
        os.system(cmd)
