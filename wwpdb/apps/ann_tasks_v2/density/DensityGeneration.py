...
class DensityConversionInSession:
    ...
    def em_volume_conversion(self, in_map, out_map):
        return self.density_wrapper.convert_em_volume(in_em_volume=in_map, out_binary_volume=out_map, working_dir=self.session_directory)

    def xray_density_conversion(self, coord_file, in_2fofc_map, in_fofc_map, out_binary_cif):
        return self.density_wrapper.convert_xray_density_map(
            coord_file=coord_file, in_2fofc_map=in_2fofc_map, in_fofc_map=in_fofc_map, out_binary_cif=out_binary_cif, working_dir=self.session_directory
        )
