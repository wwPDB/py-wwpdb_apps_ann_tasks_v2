...
from wwpdb.apps.wf_manager.admin.WfmAdmin import WfmAdmin
#
...
class CommonTasksWebAppWorker(WebAppWorkerBase):
    ...
    def _secondaryStructureCalcOp(self):
        ...
        warnFlag = calc.getLastStatus() != "ok"
        tagL = calc.getAnchorTagList(label=None, target='_blank', cssClass='')
        #
        tss = TaskSessionState(reqObj=self._reqObj, verbose=self._verbose, log=self._lfh)
    ...
    def _setSessionInfoWf(self, entryId, entryFileName):
        ...
        #
        # uds=UtilDataStore(reqObj=self._reqObj,prefix=entryId, verbose=self._verbose, log=self._lfh)
        wfFormIdTupleList = [('#solvent-task-form', 'Solvent'),
                             ('#link-task-form', 'Link'),
        ...
        try:
            ...
            if uploadFileFormId:
                ...
                for formId in uploadFileFormIdList:
                    if not formId in uploadFileFormIdMap:
                        continue
                    #
            ...
            uds = UtilDataStore(reqObj=self._reqObj, prefix=entryId, verbose=self._verbose, log=self._lfh)
            oldkeyList = ['siteargs', 'linkargs', 'solventargs', 'nafeatureargs', 'topfile',
                          'sitestatus', 'linkstatus', 'solventstatus', 'nafeaturestatus', 'secstructstatus',
                          'sitecomplete', 'linkcomplete', 'solventcomplete', 'nafeaturecomplete', 'secstructcomplete',
                          'assemanalcomplete', 'assemupdatestatus', 'assemcomplete',
                          'checkstatus', 'checkcomplete', 'valstatus', 'valcomplete', 'mapstatus', 'mapcomplete',
                          'extracheckstatus', 'extracheckcomplete', 'dccstatus', 'dcccomplete',
                          'dccrefinestatus', 'dccrefinecomplete', 'specialpositionstatus', 'specialpositioncomplete',
                          'reassignaltidsstatus', 'reassignaltidscomplete', 'bisofullstatus', 'bisofullcomplete',
                          'transcoordstatus', 'transcoordcomplete', 'terminalatomsstatus', 'terminalatomscomplete',
                          'mergexyzstatus', 'mergexyzcomplete', 'geomvalidstatus', 'geomvalidcomplete']
    ...
    def _newEntryWfOp(self):
        """  Add a new entry to the workflow system --

        """
        if (self._verbose):
            self._lfh.write("+CommonTasksWebAppWorker._newEntryWfOp() starting\n")

        self._getSession(useContext=True)

        modelFileName = self._reqObj.getValue("entryfilename")
        sfFileName = self._reqObj.getValue("entryexpfilename")
        entryId = self._reqObj.getValue("entryid")
        datasetId = self._reqObj.getValue("datasetid")
        annotator = self._reqObj.getValue("annotator")
        purgeFlag = self._reqObj.getValue("purgeflag")
        pOpt = False
        if purgeFlag == "Y":
            pOpt = True
        #
        ...
        rC.setReturnFormat("json")
        #
        # Preserve context in the return object
        #
        rC.set('entryid', self._reqObj.getValue("entryid"))
        rC.set('entryfilename', self._reqObj.getValue("entryfilename"))
        rC.set('entryexpfilename', self._reqObj.getValue("entryexpfilename"))
        #
        # check for minimum input
        #
        if ((len(datasetId) < 1) or (len(annotator) < 1) or (len(modelFileName) < 1)):
            rC.setError(errMsg="Missing require data for entry creation")
            return rC
        #
        #

        wfa = WfmAdmin(verbose=self._verbose, log=self._lfh)
        existsFlag = wfa.entryExists(datasetId)
        if (existsFlag):
            self._lfh.write("\n+CommonTasksWebAppWorker._newEntryWfOp() - resetting existing data set  %s\n" % datasetId)
            wfa.resetStatus(datasetId)
            sleep(5)

        if existsFlag and not pOpt:
            self._lfh.write("\n+CommonTasksWebAppWorker._newEntryWfOp() - data set  %s exists %r\n" % (datasetId, existsFlag))
            sTup = wfa.getStatus(id)
            self._lfh.write("\n+CommonTasksWebAppWorker._newEntryWfOp() - data set  %s status %r\n" % (datasetId, sTup))
            ok = False
        else:
            self._lfh.write("\n+CommonTasksWebAppWorker._newEntryWfOp() - adding data set  %s\n" % datasetId)
            #
            # Code to add entry here --
            #
            dE = DataExchange(reqObj=self._reqObj, depDataSetId=datasetId, wfInstanceId=None,
                              fileSource='archive', verbose=self._verbose, log=self._lfh)
            #
            if dE.createArchiveDir(purgeFlag=pOpt):
                dE.removeWorkflowDir()
                #
                #  move files -
                modelPath = os.path.join(self._sessionPath, modelFileName)
                ok = dE.export(modelPath, contentType="model", formatType="pdbx", version="next")
                if len(sfFileName) > 0:
                    sfPath = os.path.join(self._sessionPath, sfFileName)
                    ok = dE.export(sfPath, contentType="structure-factors", formatType="pdbx", version="next")
                #
                wfa.deleteStatus(datasetId)
                wfa.initializeEntry(datasetId, annotator)
                sTup = wfa.getStatus(datasetId)
                self._lfh.write("+CommonTasksWebAppWorker._newEntryWfOp() dataset %s status %r\n" % (datasetId, sTup))
                ok = True
            else:
                if (self._verbose):
                    self._lfh.write("+CommonTasksWebAppWorker._newEntryWfOp() archive directory creation failed\n")
                ok = False

        if (self._verbose):
            self._lfh.write("+CommonTasksWebAppWorker._newEntryWfOp() status %r\n" % ok)
        #
        #
        if ok:
            rC.setHtmlText("New entry added")
        else:
            rC.setError(errMsg="New entry addition failed")
        return rC

    def _resetEntryWfOp(self):
        """  Reset the status of an entry in the workflow system --
        """
        if (self._verbose):
            self._lfh.write("+CommonTasksWebAppWorker._resetWfOp() starting\n")

        self._getSession(useContext=True)

        modelFileName = self._reqObj.getValue("entryfilename")
        sflFileName = self._reqObj.getValue("entryexpfilename")
        entryId = self._reqObj.getValue("entryid")
        datasetId = self._reqObj.getValue("datasetid")
        annotator = self._reqObj.getValue("annotator")
        purgeFlag = self._reqObj.getValue("purgeflag")
        ...
        if existsFlag:
            self._lfh.write("\n+CommonTasksWebAppWorker._resetWfOp() - data set  %s exists %r\n" % (datasetId, existsFlag))
            sTup = wfa.getStatus(datasetId)
            self._lfh.write("\n+CommonTasksWebAppWorker._resetWfOp() - data set  %s status %r\n" % (datasetId, sTup))

            self._lfh.write("\n+CommonTasksWebAppWorker._resetWfOp() - resetting data set  %s\n" % datasetId)
            # code to reset data set here --
            dE = DataExchange(reqObj=self._reqObj, depDataSetId=datasetId, wfInstanceId=None,
                              fileSource='archive', verbose=self._verbose, log=self._lfh)
            if (pOpt):
                dE.reversePurge(contentType="model", formatType="pdbx")
                dE.reversePurge(contentType="structure-factors", formatType="pdbx")
                dE.purgeLogs()
                dE.removeWorkflowDir()

            self._lfh.write("\n+CommonTasksWebAppWorker._resetWfOp() - running resetting data set  %s\n" % datasetId)
            wfa.resetStatus(datasetId)
            sTup = wfa.getStatus(datasetId)
            self._lfh.write("+CommonTasksWebAppWorker._resetWfOp() dataset %s status %r\n" % (datasetId, sTup))
            sDTup = wfa.getStatusDetails(datasetId)
            self._lfh.write("+CommonTasksWebAppWorker._resetWfOp() dataset %s status %r\n" % (datasetId, sDTup))
            ok = True
        else:
            self._lfh.write("\n+CommonTasksWebAppWorker._resetWfOp() - data set  %s exists %r cannot reset\n" % (datasetId, existsFlag))
            ok = False

        if (self._verbose):
            self._lfh.write("+CommonTasksWebAppWorker._resetWfOp() status %r\n" % ok)
        #
        rC = ResponseContent(reqObj=self._reqObj, verbose=self._verbose, log=self._lfh)
        rC.setReturnFormat("json")
        rC.set('entryid', self._reqObj.getValue("entryid"))
        ...
        if ok:
            rC.setHtmlText("Data set status reset")
        else:
            rC.setError(errMsg="Data set status reset failed")

        return rC

    def _reassignEntryWfOp(self):
        """  Reassign the annotator for an  entry in the workflow system --
        """
        if (self._verbose):
            self._lfh.write("+CommonTasksWebAppWorker._reassignWfOp() starting\n")
        ...
        existsFlag = wfa.entryExists(datasetId)
        if existsFlag:
            self._lfh.write("\n+CommonTasksWebAppWorker._reassignWfOp() - data set  %s exists %r\n" % (datasetId, existsFlag))
            sTup = wfa.getStatus(datasetId)
            self._lfh.write("\n+CommonTasksWebAppWorker._reassignWfOp() - data set  %s status %r\n" % (datasetId, sTup))

            self._lfh.write("\n+CommonTasksWebAppWorker._reassignWfOp()  - reassigning data set %s to %s \n" % (datasetId, annotator))
            wfa.assignAnnotator(datasetId, annotator)
            ok = True
        else:
            self._lfh.write("\n+CommonTasksWebAppWorker._reassignWfOp() - data set  %s exists %r cannot reassign\n" % (datasetId, existsFlag))
            ok = False

        if (self._verbose):
            self._lfh.write("+CommonTasksWebAppWorker._reassignWfOp() status %r\n" % ok)
        #
        rC = ResponseContent(reqObj=self._reqObj, verbose=self._verbose, log=self._lfh)
        rC.setReturnFormat("json")
        rC.set('entryid', self._reqObj.getValue("entryid"))
        ...
        if ok:
            rC.setHtmlText("Data set reassigned")
        else:
            rC.setError(errMsg="Data set reassignment failed")
        return rC
    ...
    def _entryInfoOp(self):
        ...
        kyPairList = [('struct_title', 'struct_title'),
                      ('pdb_id', 'pdb_id'),
                      ('emdb_id', 'emdb_id'),
                      ('bmrb_id', 'bmrb_id'),
    ...
    def _downloadResponderOp(self):
        ...
        self._getSession(useContext=True)
        entryId = self._reqObj.getValue("entryid")
        fileSource = self._reqObj.getValue("file_source")
    ...
    def __uploadConversion(self, entryId, inpFileName, inpContentType, inpFormatType, cnvOp, outFileName, outContentType, outFormatType, timeOut=0):
        ...
        if inpContentType == 'model':
            ...
            elif (fileType == "rcsb-mmcif-strip-plus-entity"):
                ok = dfa.rcsb2Pdbx(filePath, filePath, stripFlag=True, stripEntityFlag=True)
            elif (fileType == "rcsb-cifeps-strip-plus-entity"):
