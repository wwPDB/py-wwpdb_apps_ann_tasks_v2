...
class MapDisplay(object):
    ...
    def __setup(self):
        ...
        self.__attribNameList=['id' ,'residue_name' ,'chain_id'  ,'dcc_correlation ' ,'real_space_R'  ,'Biso_mean ','occupancy_mean'  ,
                               'warning','file_name_map_html'        ,'file_name_pdb'        ,'file_name_map'        ,'file_name_jmol']
    ...
    def renderLocalMapTable(self,rowDL,title='Table of local electron density maps for non-polymer chemical components', subdir='np-cc-maps'):
        """  
        """
        attribNameList=['id' ,'residue_name' ,'chain_id'  ,'dcc_correlation ' ,'real_space_R'  ,'Biso_mean ','occupancy_mean'  ,
                        'warning','file_name_map_html'        ,'file_name_pdb'        ,'file_name_map'        ,'file_name_jmol']

        colList=['id' ,'residue_name' ,'chain_id'  ,'dcc_correlation' ,'real_space_R'  ,'Biso_mean','occupancy_mean'  ,
                        'warning','file_name_map_html'        ,'file_name_pdb'        ,'file_name_map'        ,'file_name_jmol']

        colDisplayD={'id':'View in JSMol',
        ...
        oL.append('<tr>')
        for col in colList:
            if not col in ['id', 'residue_name', 'chain_id', 'dcc_correlation', 'real_space_R', 'Biso_mean', 'occupancy_mean']:
                continue
            oL.append('<th>')
