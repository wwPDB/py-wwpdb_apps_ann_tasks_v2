...
class SpecialPositionUpdate(SessionWebDownloadUtils):
    ...
    def run(self,entryId,inpFile,updateInput=True): 
        ...
        try:
            ...
            #
            if (self.__verbose):
                self.__lfh.write("+%s.%s special position update calc completed for entryId %s file %s\n" % (self.__class__.__name__,sys._getframe().f_code.co_name,entryId,inpFile))
            ...
            if (self.__verbose):
                self.__lfh.write("+%s.%s special position update calc failed for entryId %s file %s\n" % (self.__class__.__name__,sys._getframe().f_code.co_name,entryId,inpFile))
