...
class PdbxGeometryReportIo(PdbxStyleIoUtil):
    ...
    def __init__(self, verbose=True, log=sys.stderr):
        super(PdbxGeometryReportIo, self).__init__(styleObject=PdbxGeometryReportCategoryStyle(), verbose=verbose,
                                                   log=log)
...
class PdbxXrayExpReportIo(PdbxStyleIoUtil):
    ...
    def __init__(self, verbose=True, log=sys.stderr):
        super(PdbxXrayExpReportIo, self).__init__(styleObject=PdbxXrayExpReportCategoryStyle(), verbose=verbose,
                                                  log=log)
...
class PdbxStatusHistoryIo(PdbxStyleIoUtil):
    ...
    def __init__(self, verbose=True, log=sys.stderr):
        super(PdbxStatusHistoryIo, self).__init__(styleObject=PdbxStatusHistoryCategoryStyle(), verbose=verbose,
                                                  log=log)
...
class PdbxEntryInfoIo(PdbxStyleIoUtil):
    ...
    def __getStatusDetails(self):
        ...
        mappings = {'status_code': 'status_code',
    ...
    def setStatusDetails(self, statusCode, approvalType, annotatorInitials):
        ...
        try:
            ...
            if (self.__verbose):
                logger.info(
                    "+PdbxEntryInfo.setStatusDetails() failed for status code %s approval type %s initials %s\n" %
    ...
    def __getInfoGeneral(self):
        ...
        #
        oD = {}
        kys = ['blockId', 'pdb_id', 'bmrb_id', 'emdb_id', 'experimental_methods', 'struct_title', 'status_code',
               'auth_release_code',
               'deposit_date', 'hold_coord_date', 'coord_date', 'approval_type', 'annotator_initials', 'process_site',
               'deposit_site',
               'reqacctypes']
        for ky in kys:
            oD[ky] = ''
    ...
    def __getInfoHistory(self):
        ...
        try:
            ...
            if catObj is not None:
                ...
                authReleaseCode = catObj.getValueOrDefault(attributeName='author_release_status_code', rowIndex=0,
                                                           defaultValue='')
                initialDepositionDate = catObj.getValueOrDefault(attributeName='recvd_initial_deposition_date',
                                                                 rowIndex=0, defaultValue='')
                holdCoordinatesDate = catObj.getValueOrDefault(attributeName='date_hold_coordinates', rowIndex=0,
                                                               defaultValue='')
                coordinatesDate = catObj.getValueOrDefault(attributeName='date_coordinates', rowIndex=0,
                                                           defaultValue='')
                approvalType = catObj.getValueOrDefault(attributeName='author_approval_type', rowIndex=0,
                                                        defaultValue='')
                annotatorInitials = catObj.getFirstValueOrDefault(
                    attributeNameList=['pdbx_annotator', 'rcsb_annotator'], rowIndex=0, defaultValue='')
                beginProcessingDate = catObj.getValueOrDefault(attributeName='date_begin_processing', rowIndex=0,
                                                               defaultValue='')
                authorApprovalDate = catObj.getValueOrDefault(attributeName='date_author_approval', rowIndex=0,
                                                              defaultValue='')
...
class PdbxLocalMapIndexIo(PdbxStyleIoUtil):
    ...
    def __init__(self, verbose=True, log=sys.stderr):
        super(PdbxLocalMapIndexIo, self).__init__(styleObject=PdbxLocalMapIndexCategoryStyle(), verbose=verbose,
                                                  log=log)
