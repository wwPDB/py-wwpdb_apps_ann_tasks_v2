...
class PdbxReportDepictBootstrap(PdbxDepictBootstrapBase):
    ...
    def __setup(self):
        ...
        if self.__st.getStyleId() in ["PDBX_T1"] :
            ...
            self.__reportCategories = [
                ...
                ('pdbx_entity_branch', 'Branched entities', 'row-wise'),
    ...
    def __doRenderTabs(self,eD):
        ...
        tableList=self.__reportCategories
        #
    ...
    def __markupRow(self,catName, rD):
        ...
        if catName == 'pdbx_reference_molecule_synonyms':
            ...
            if itemName in rD:
                itemValue=rD[itemName]
                if len(itemValue) >=3:
                    rD[itemName]='<a target="_blank" href="http://ligand-expo.rcsb.org/pyapps/ldHandler.py?formid=cc-index-search&operation=ccid&target=%s">%s</a>' % (itemValue,itemValue)
