...
class MtzTommCIF(SessionWebDownloadUtils):
    ...
    def __bashSetting(self):
        setting = " PACKAGE_DIR=" + self.__packagePath + "; export PACKAGE_DIR; " \
    ...
    def __generateInputForm(self, logPath):
        """
        """
        options = " -mtz_man_html " + self.__mtzDataSet + " -url_users_data " + os.path.join("/sessions", self.__sessionId) \
                + " -users_data " + self.__topSessionPath + " -cgi_bin  /cgi-bin/ -sf " + self.__mtzFileName + " > " + logPath + " 2>&1 ; "
    ...
    def __convertMtzTommCIF(self, expFileName, retPath, logPath):
        ...
        #
        options = " -i mtz -o mmcif -sf " + self.__mtzFileName + " -out " + retPath + " -pdb_id " + pdbid \
                + " -label " + labels + " > " + logPath + " 2>&1 ; "
        self.__runCmd(options)
        #
    ...
    def __readHtmlText(self):
        ...
        try:
            ...
            for line in data.split("\n"):
                ...
                #    break
                elif (not start) or line.startswith("<form ENCTYPE=") or line.startswith("<INPUT TYPE=SUBMIT NAME=") or \
                      line.startswith("<INPUT TYPE=RESET VALUE") or line.startswith("</form>"):
                    continue
                #
    ...
    def __parseSemiAutoForm(self):
        """
        """
        tokenNameList = [ [ "fp", " 'FP=%s', " ], [ "sigfp", " 'SIGFP=%s', " ], [ "i", " 'I=%s', " ], [ "sigi", " 'SIGI=%s', " ], \
                          [ "free", " 'FREE=%s', " ], [ "phib", " 'PHIB=%s', " ], [ "fom", " 'FOM=%s', " ], [ "fc", " 'FC=%s', " ], \
                          [ "phic", " 'PHIC=%s', " ], [ "hla", " 'HLA=%s', " ], [ "hlb", " 'HLB=%s', " ], [ "hlc", " 'HLC=%s', " ], \
                          [ "hld", " 'HLD=%s', " ], [ "iplus", " 'I(+)=%s', " ], [ "sigiplus", " 'SIGI(+)=%s', " ], \
                          [ "ineg", " 'I(-)=%s', " ], [ "sigineg", " 'SIGI(-)=%s', " ], [ "fplus", " 'F(+)=%s', " ], \
                          [ "sigfplus", " 'SIGF(+)=%s', " ], [ "fneg", " 'F(-)=%s', " ], [ "sigfneg", " 'SIGF(-)=%s', " ], \
                          [ "dp", " 'DP=%s', " ], [ "sigdp", " 'SIGDP=%s', " ], [ "fwt", " 'FWT=%s', " ], [ "phwt", " 'PHWT=%s', " ], \
                          [ "delfwt", " 'DELFWT=%s', " ], [ "delphwt", " 'DELPHWT=%s', " ], [ "freer", " -FREER %s , " ] ]
        #
        items = ""
