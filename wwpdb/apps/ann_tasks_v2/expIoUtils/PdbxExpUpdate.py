    ...
    def doUpdate(self,entryId,modelInputFile=None,expInputFile=None,expOutputFile=None):
        ...
        try:
            if modelInputFile is None:
                ...
                modeFilePath=os.path.join(self.__sessionPath,modelFileName)
            else:
            ...
            for container in containerList:
                ...
                for dId in diffrnIdList:
                    if (not dId in modelWavelengthD) or (not modelWavelengthD[dId]):
                        continue
                    #
                ...
                exp_wavelength_1 = ""
                cleanFlag = False
                if curContainerName in self.__inputExpWvMap and self.__inputExpWvMap[curContainerName] and \
                   curContainerName in self.__inputExpMuMap and self.__inputExpMuMap[curContainerName]:
                ...
                if diffFlag:
                    ...
                    self.__warningMsg.append("Wavelength conflict between model coordinate file and '" + curContainerName \
                                           + "' data block in reflection data file.")
                #
                if dataInfo:
